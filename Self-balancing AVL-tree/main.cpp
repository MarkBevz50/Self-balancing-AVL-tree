...
int main() {
    ...
    std::cin >> condition;
    try {
        root = deleteNode(root, condition); // Видалення введеного елемента з дерева

        std::cout << "AVL Tree (Graph representation) after deletion:\n";
        printTreeGraph(root, 0, 10); // Виведення графічного представлення дерева після видалення
    }
